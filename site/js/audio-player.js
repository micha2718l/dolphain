...
class AudioPlayer {
  ...
  setMode(mode) {
    ...
    const audioSrc =
      mode === "raw" ? this.fileData.audio_raw : this.fileData.audio_denoised;
    const specSrc =
      mode === "raw"
        ? this.fileData.spectrogram_raw
        : this.fileData.spectrogram_denoised;
    const wavSrc =
      mode === "raw"
        ? this.fileData.waveform_raw
        : this.fileData.waveform_denoised;
  ...
  seekTo(percent) {
    ...
    const seekedHandler = () => {
      ...
      if (Math.abs(actualTime - this.targetSeekTime) > 0.5) {
        ...
        this.audioElement.addEventListener("seeked", seekedHandler, {
          once: true,
        });
        return;
      }
