...
def detect_click_trains(data, fs, click_freq_range=(20000, 150000), 
                       min_clicks=10, max_ici=0.05):
    """
    ...
    peak_indices, properties = sp_signal.find_peaks(
        ...
        width=(1, int(fs * 0.005))   # Width between 1 sample and 5ms (reject wide bumps)
    )
        ...
        if ici <= max_ici:
            ...
            if len(current_train) >= min_clicks:
                ...
                click_trains.append({
                    ...
                    'std_ici': np.std(icis),
                    'click_rate': len(train_data) / (train_data[-1] - train_data[0]) if train_data[-1] > train_data[0] else 0,
...
def quick_find(
    ...
    for i, file_path in enumerate(files_to_process):
        ...
        try:
            ...
            click_trains = detect_click_trains(
                signal_clean,
                data_dict["fs"],
                click_freq_range=(20000, min(data_dict["fs"] // 2, 150000)),  # Higher frequency minimum
        ...
        if (overall_index + 1) % 10 == 0 or (overall_index + 1) == total_files:
            ...
            if files_done > 0:
                ...
                n_with_both = sum(1 for r in results if r.get("n_chirps", 0) > 0 and r.get("n_click_trains", 0) > 0)
    ...
    print(f"Files with click trains: {n_with_clicks} ({n_with_clicks/len(results)*100:.1f}%)")
