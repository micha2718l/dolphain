...
def main():
    ...
    parser.add_argument(
        "--top", type=int, default=25, help="Number of top files to copy"
    )
    parser.add_argument(
        "--output-dir", default="temp_showcase_files", help="Directory to copy files to"
    )
    ...
    manifest = {
        ...
        "copied_files": [
            str(output_dir / Path(f).name)
            for f in file_list
            if (output_dir / Path(f).name).exists()
        ],
    }
    ...
    print(
        f"   python scripts/generate_showcase_local.py --input-dir {output_dir} --top {args.top}"
    )
