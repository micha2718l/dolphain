...
def create_spectrogram_image(audio_data, sample_rate, output_path, title=""):
    ...
    plt.tight_layout()
    plt.savefig(
        output_path, dpi=80, facecolor="#0a0e27", edgecolor="none"
    )  # Reduced from dpi=100
    plt.close()
...
def main():
    ...
    parser.add_argument(
        "--input-dir", required=True, help="Directory with copied EARS files"
    )
    parser.add_argument(
        "--top", type=int, default=25, help="Number of files to process"
    )
    parser.add_argument(
