...
def main():
    ...
    parser.add_argument('--showcase-dir', default='site/showcase', help='Showcase directory')
    ...
    for wav_file in wav_files:
        ...
        if convert_to_mp3(wav_file, mp3_file, args.bitrate):
            ...
            print(f"✅ {wav_size // 1024 // 1024}MB → {mp3_size // 1024 // 1024}MB ({compression:.0f}% smaller)")
            converted += 1
        else:
            print("❌ Failed")
    ...
    if data_file.exists():
        ...
        for file_info in data['files']:
            ...
            file_info['audio_denoised'] = file_info['audio_denoised'].replace('.wav', '.mp3')
    ...
    print(f"🎉 Saved {(total_wav_size - total_mp3_size) // 1024 // 1024} MB ({((total_wav_size - total_mp3_size) / total_wav_size * 100):.0f}% reduction)")
    print(f"{'='*60}")
